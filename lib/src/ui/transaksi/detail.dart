                ...
                children: [
                  ...
                  ...args.object!.details!.map(
                    (e) => Column(
                      children: [
                        ...
                        _itemDetailTransaksi(
                          context,
                          '${e.quantity ?? '-'} x ${toRupiah(double.parse(e.price.toString() ?? "0"))}',
                          toRupiah(
                            double.parse(
                              (int.parse(e.price.toString() ?? "0") *
                                      int.parse(e.quantity.toString() ?? "0"))
                                  .toString(),
                            ),
                          ),
                          titleBold: false,
                  ...
                  _itemDetailTransaksi(
                    ...
                    toRupiah(double.parse(
                        args.object!.totalPrice.toString() ?? "0")),
                  ),
                  if (args.object!.amountPaid != '0')
                    _itemDetailTransaksi(
                      ...
                      'Jumlah Bayar',
                      toRupiah(double.parse(
                          args.object!.amountPaid.toString() ?? "0")),
                    ),
                  if (args.object!.amountPaid != '0')
                    _itemDetailTransaksi(
                      context,
                      ...
                      toRupiah(
                        double.parse(
                          (int.parse(args.object!.amountPaid.toString() ??
                                      "0") -
                                  int.parse(
                                      args.object!.totalPrice.toString() ??
                                          "0"))
                              .toString(),
                        ),
                      ),
                ...
                onPressed: () async {
                  ...
                  final List<CartDBModel> cart = [
                    ...args.object!.details!.map((e) => CartDBModel.fromMap({
                          '_id': int.parse(e.itemId.toString()!),
                          'name': e.item!.name,
                          'unit': e.item!.unit,
                          'price': int.parse(e.price.toString()!),
                          'qty': int.parse(e.quantity.toString()!),
                        }))
                  ];
                  ...
                  print1.sample(
                    ...
                    total: int.parse(args.object!.totalPrice.toString() ?? '0'),
                    buyerName: args.object!.name ?? '',
                    amountPaid: args.object!.amountPaid.toString() ?? '0',
